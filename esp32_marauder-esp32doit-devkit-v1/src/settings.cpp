...
String settings_string; // Declare the settings_string variable
...
bool Settings::begin() {
  ...
  #include <ArduinoJson.h> // Include the necessary header file

  String json_string;
  DynamicJsonDocument jsonBuffer(1024); // Adjust the constructor argument to match the required size
...
int Settings::loadSetting<int>(String key) {
  ...
  if (deserializeJson(jsonBuffer, this->json_settings_string)) {
    Serial.println("\nCould not parse json");
}

  for (int i = 0; i < jsonBuffer["Settings"].size(); i++) {
    if (jsonBuffer["Settings"][i]["name"].as<String>() == key)
      return jsonBuffer["Settings"][i]["value"];
  }
...
bool Settings::saveSetting<bool>(String key, bool value) {
  ...
  for (int i = 0; i < jsonBuffer["Settings"].size(); i++) {
    if (jsonBuffer["Settings"][i]["name"].as<String>() == key) {
      jsonBuffer["Settings"][i]["value"] = value;
...
bool Settings::toggleSetting(String key) {
  ...
  for (int i = 0; i < jsonBuffer["Settings"].size(); i++) {
    if (jsonBuffer["Settings"][i]["name"].as<String>() == key) {
      if (jsonBuffer["Settings"][i]["value"]) {
...
String Settings::setting_index_to_name(int i) {
  ...
  return jsonBuffer["Settings"][i]["name"];
}
...
int Settings::getNumberSettings() {
  ...
  return jsonBuffer["Settings"].size();
}

String Settings::getSettingType(String key) {
  DynamicJsonDocument jsonBuffer(1024); // Use StaticJsonDocument instead of JsonDocument
  ...
  for (int i = 0; i < jsonBuffer["Settings"].size(); i++) {
    if (jsonBuffer["Settings"][i]["name"].as<String>() == key)
      return jsonBuffer["Settings"][i]["type"];
  }
}
...
void Settings::printJsonSettings(String json_string) {
  ...
  if (deserializeJson(jsonBuffer, json_string)) {
    Serial.println("\nCould not parse json");
  }

  Serial.println("Settings\n----------------------------------------------");
  for (int i = 0; i < jsonBuffer["Settings"].size(); i++) {
    Serial.println("Name: " + jsonBuffer["Settings"][i]["name"].as<String>());
    Serial.println("Type: " + jsonBuffer["Settings"][i]["type"].as<String>());
    Serial.println("Value: " + jsonBuffer["Settings"][i]["value"].as<String>());
    Serial.println("----------------------------------------------");
  }
}
...
bool Settings::createDefaultSettings(fs::FS &fs) {
  ...
  // Adjust the constructor argument to match the required size
